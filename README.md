...
> 현재 이 저장소는 MVP 개발 예정 단계입니다. 아래 내용은 `edge_ai_kalman_dashboard_proposal.md`를 기준으로 정리한 구현 계획이며, 아직 완성된 서비스나 배포된 결과를 의미하지 않습니다.
...
|---|---|---|
| 논문 4.3.1 RMSE | `lib/metrics.ts#calculateRMSE` | Planned |
| 논문 4.3.1 MAE | `lib/metrics.ts#calculateMAE` | Planned |
| 논문 4.3.1 NIS pass rate | `lib/metrics.ts#calculateNISPassRate` | Planned |
| 논문 4.3.1 R 추정 RMSE | `lib/metrics.ts#calculateRRMSE` | Planned |
| 논문 4.3.1 수렴 시간 | `lib/metrics.ts#calculateTconv` | Planned |
| CSV 18컬럼 파싱 및 검증 | `lib/csv-parser.ts` | Planned |
| E1 정상 baseline 비교 | `components/views/E1View.tsx` | Planned |
| E3 차단 구간 강조 | `components/views/E3View.tsx` | Planned |
| Ablation feature set 비교 | `app/ablation/page.tsx` | Planned |
...
`/dashboard`는 시나리오별 페이지를 따로 나누지 않고, 내부 컴포넌트에서 E0~E5를 분기하는 구조를 계획합니다.
